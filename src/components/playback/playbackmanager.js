...
import { SubtitleDiagnosticTrace } from 'plugins/htmlVideoPlayer/subtitles/diagnostics';
...
export class PlaybackManager {
    constructor() {
        ...
        self.setSubtitleStreamIndex = function (index, player) {
            ...
            new SubtitleDiagnosticTrace({ slot: 0, trackIndex: index }).record('manager-selection', {
                codec: newStream?.Codec?.toLowerCase(), playMethod: self.playMethod(player),
                delivery: newStream ? getDeliveryMethod(newStream) : undefined,
                previousDelivery: currentStream ? getDeliveryMethod(currentStream) : undefined
            });
        ...
        function changeStream(player, ticks, params) {
            ...
            const diagnostic = new SubtitleDiagnosticTrace({ trackIndex: params.SubtitleStreamIndex });
            diagnostic.record('video-stream-change');
            ...
            player.getDeviceProfile(currentItem, {
                ...
                diagnostic.record('video-device-profile-ready');
                ...
                diagnostic.record('video-playback-info-start');
                getPlaybackInfo(player, apiClient, currentItem, deviceProfile, currentMediaSource.Id, liveStreamId, options).then(function (result) {
                    diagnostic.record('video-playback-info-ready');
        ...
        function changeStreamToUrl(apiClient, player, playSessionId, streamInfo) {
            const playerData = getPlayerData(player);
            const diagnostic = new SubtitleDiagnosticTrace();
            diagnostic.record('video-source-replacement');
            ...
            if (playerData.streamInfo && playSessionId) {
                apiClient.stopActiveEncodings(playSessionId).then(function () {
                    diagnostic.record('video-old-encoding-stopped');
