...
function hideSelections() {
    ...
    if (selectionCommandsPanel) {
        ...
        const elems = selectionContainer?.querySelectorAll('.itemSelectionPanel') || [];
...
function showSelections(initialCard, addInitialCheck, isCurrent = () => true) {
    const container = initialCard.closest('.itemsContainer');
    const isAvailable = () => container?.isConnected && container.contains(initialCard)
        && !initialCard.closest('[inert]') && initialCard.getClientRects().length > 0
        && window.getComputedStyle(initialCard).visibility !== 'hidden';
    if (!isAvailable() || !isCurrent()) return;
    if (selectionContainer && selectionContainer !== container) hideSelections();
    pendingContainer = container;
    const generation = ++selectionGeneration;
    import('../../elements/emby-checkbox/emby-checkbox').then(() => {
        if (generation !== selectionGeneration || !isCurrent() || !isAvailable()) return;
        pendingContainer = null;
        ...
        selectionObserver = new MutationObserver(() => {
            if (selectedElements.some(element => !container.contains(element))) hideSelections();
        });
        selectionObserver.observe(container, { childList: true, subtree: true });
    }).catch(err => {
        if (generation === selectionGeneration) hideSelections();
        console.error('[MultiSelect] failed to load selection controls', err);
    });
}
...
function onContainerClick(e) {
    ...
    if (selectedItems.length && selectionContainer === this) {
...
document.addEventListener('pagebeforeshow', hideSelections);
window.addEventListener('popstate', hideSelections);
...
export default function (options) {
    ...
    function selectCard(card) {
        ...
        showSelections(card, true, () => !destroyed && generation === holdGeneration);
    }

    function onPageChange() {
        onMouseOut();
        if (selectionContainer === container || pendingContainer === container) hideSelections();
    }
    ...
    document.addEventListener('pointercancel', onMouseOut);
    document.addEventListener('visibilitychange', onPageChange);
    document.addEventListener('viewbeforehide', onPageChange);
    ...
    self.destroy = () => {
        ...
        document.removeEventListener('pointercancel', onMouseOut);
        document.removeEventListener('visibilitychange', onPageChange);
        document.removeEventListener('viewbeforehide', onPageChange);
        document.removeEventListener('pagebeforeshow', onPageChange);
        window.removeEventListener('blur', onPageChange);
