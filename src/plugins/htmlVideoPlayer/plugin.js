...
export class HtmlVideoPlayer {
    ...
    /**
     * @type {FinwebPlaybackSession | null | undefined}
     */
    ...
    async play(options) {
        ...
        const presentationGeneration = this.#subtitlePlaybackGeneration;
        ...
        try {
            ...
            if (presentationGeneration !== this.#subtitlePlaybackGeneration) return;
            ...
            if (presentationGeneration === this.#subtitlePlaybackGeneration) {
    ...
    setSrcWithFlvJs(elem, options, url) {
        const generation = this.#subtitlePlaybackGeneration;
        return import('flv.js').then(({ default: flvjs }) => {
            if (generation !== this.#subtitlePlaybackGeneration) return;
    ...
    setSrcWithHlsJs(elem, options, url) {
        ...
        return new Promise((resolve, reject) => {
            requireHlsPlayer(async () => {
                if (generation !== this.#subtitlePlaybackGeneration) return resolve();
    ...
    async setCurrentSrc(elem, options) {
        ...
        if (this.#subtitleTrackIndexToSetOnPlaying != null && this.#subtitleTrackIndexToSetOnPlaying >= 0) {
            const initialSubtitleStream = options.mediaSource.MediaStreams.find(stream => stream.Index === this.#subtitleTrackIndexToSetOnPlaying && stream.Type === 'Subtitle');
        ...
        if (secondaryTrackValid) {
            ...
            if (this.#secondarySubtitleTrackIndexToSetOnPlaying != null && this.#secondarySubtitleTrackIndexToSetOnPlaying >= 0) {
                const initialSecondarySubtitleStream = options.mediaSource.MediaStreams.find(stream => stream.Index === this.#secondarySubtitleTrackIndexToSetOnPlaying && stream.Type === 'Subtitle');
        ...
        // Start subtitle preparation with the source, independent of playing/OSD
        // navigation. A later user selection must never be reset by navigation.
        this.getTextSubtitlePipeline(elem);
        this.setCurrentTrackElement(this.#subtitleTrackIndexToSetOnPlaying);
        if (this.#secondarySubtitleTrackIndexToSetOnPlaying >= 0) {
            this.setSecondarySubtitleStreamIndex(this.#secondarySubtitleTrackIndexToSetOnPlaying);
        }
        ...
        if (!this.#started) {
            ...
            if (this._currentPlayOptions.fullscreen) {
                ...
                appRouter.showVideoOsd().then(() => {
                    if (generation === this.#subtitlePlaybackGeneration) this.onNavigatedToOsd();
                });
            } else {
            ...
            this.#textSubtitlePipeline = new FinwebPlaybackSession({
                ...
                onStateChange: ({ slot, trackIndex, state, error, restoredTrackIndex }) => {
                    ...
                    if (state === 'failed' && restoredTrackIndex !== undefined) {
                        toast(globalize.translate('FinwebSubtitleLoadFailed'));
                        queueMicrotask(() => {
                            if (playbackGeneration !== this.#subtitlePlaybackGeneration) return;
                ...
                useManagedTrack: track => this.shouldUseManagedTextRenderer(track, this._currentPlayOptions.item),
                burnInWhenTranscoding: () => appSettings.alwaysBurnInSubtitleWhenTranscoding(),
                getCueLine: slot => {
                    const line = parseInt(userSettings.getSubtitleAppearanceSettings().verticalPosition, 10);
                    return (Number.isFinite(line) ? line : -1) - (slot === SECONDARY_TEXT_TRACK_INDEX ? 3 : 0);
                },
                prepareManaged: slot => {
                    this.destroyNativeTracks(videoElement, slot);
                    this.endPendingSubtitleLoad(slot);
                    if (this.isPrimaryTrack(slot)) {
                        this.#currentBitmapSubRenderer?.dispose();
                        this.#currentBitmapSubRenderer = null;
                    }
                    this.setCueAppearance();
                },
                renderLegacy: (track, slot) => {
                    if (!track) {
                        this.destroyCustomTrack(videoElement, slot === PRIMARY_TEXT_TRACK_INDEX ? undefined : slot);
                    } else {
                        this.renderLegacyTrack(videoElement, track, this._currentPlayOptions.item, slot)
                            .catch(error => console.error('Unable to load legacy subtitle', error));
                    }
                }
            });
        }
    ...
    setCurrentTrackElement(streamIndex, targetTextTrackIndex) {
        const target = targetTextTrackIndex ?? PRIMARY_TEXT_TRACK_INDEX;
        this.#subtitleTrackSelectionTokens.set(target, Symbol(String(streamIndex)));
        if (!this.#mediaElement || !this._currentPlayOptions) return;
        return this.getTextSubtitlePipeline(this.#mediaElement).selectStream(streamIndex, target);
    }
