...
export class TextSubtitlePipeline {
    ...
    cancelPending(slot: SubtitleSlot) {
        const state = this.slots.get(slot);
        if (!state) return;
        state.loading?.cancel();
        state.loading = undefined;
        state.desiredTrackIndex = state.active?.trackIndex;
    }
        ...
        const promise = Promise.resolve().then(async () => {
            ...
            try {
                ...
                if (request.diagnostic) {
                    await request.diagnostic.measure('renderer-activate', async () => renderer!.activate(snapshot));
                } else {
