...
const server = createServer((request, response) => {
    ...
    response.end(request.url === '/fixture.js' ? bundle.outputFiles[0].text : `<!doctype html><html data-finweb-theme dir="ltr"><style>${baseCss}${finwebCss}body{margin:30px;font:16px sans-serif}.hide{display:none}</style><body><script>document.registerElement=(_,options)=>{window.sliderPrototype=options.prototype;};</script><script src="/fixture.js"></script></body></html>`);
});
...
try {
    ...
    console.log('PASS OSD seek/volume fixed 8x20 geometry, 10-to-20px scale/fade at 200ms, 4px insets, LTR/RTL alignment and pointer/keyboard input; no screenshots');
} finally {
    await browser.close();
